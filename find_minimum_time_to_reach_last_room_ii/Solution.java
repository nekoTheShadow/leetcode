...
public class Solution {
  public int minTimeToReach(int[][] moveTime) {
    ...
    while (pq.poll() instanceof Tuple(int x, int y, int t, int c)) {
      if (visited[x][y] < t) {
        continue;
      }
      ...
      for (int[] d : new int[][] {{1, 0}, {-1, 0}, {0, 1}, {0, -1}}) {
        ...
        int nt = Math.max(t + 1 + c % 2, moveTime[nx][ny] + 1 + c % 2);
        if (nt < visited[nx][ny]) {
          visited[nx][ny] = nt;
          pq.add(new Tuple(nx, ny, nt, c + 1));
        }
      }
    }
  ...
  private record Tuple(int x, int y, int t, int c) {
