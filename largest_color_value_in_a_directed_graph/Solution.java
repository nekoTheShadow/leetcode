package largest_color_value_in_a_directed_graph;
...
public class Solution {
    public int largestPathValue(String colors, int[][] edges) {
        ...
        // Cycle Check
        Tsort ts = new Tsort(n);
        for (int[] edge : edges) {
            ts.add(edge[0], edge[1]);
        }
        if (ts.hasCycle()) {
            return -1;
        }
        
        List<List<Integer>> g = IntStream.range(0, n).mapToObj(unused -> new ArrayList<Integer>()).collect(Collectors.toList());
        for (int[] edge : edges) {
        ...
        int[][] dp = new int[n][26];
        for (int cur : ts.run()) {
            if (g.get(cur).isEmpty()) {
                dp[cur][colors.charAt(cur)-'a']++;
                continue;
            }
            
            for (int pre : g.get(cur)) {
              for (int i = 0; i < 26; i++) {
                  if (i == (colors.charAt(cur)-'a')) {
...
//        
        // 3
        System.out.println(s.largestPathValue("hhqhuqhqff", new int[][] {{0,1},{0,2},{2,3},{3,4},{3,5},{5,6},{2,7},{6,7},{7,8},{3,8},{5,8},{8,9},{3,9},{6,9}}));
    }
}
